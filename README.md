...
`--allowlist file.json` lists tool calls that run without approval. Anything unmatched becomes an approval row. A `shell` rule gives an `argv` prefix; a file-tool rule gives a `path` glob relative to the thread directory, so `{ "tool": "read_file", "path": "**" }` lets a daemon read anything there. Globs match dotfiles too; scope the rule to a subtree (`notes/**`) to keep a root `.env` behind approval. Any other tool, such as one from an MCP server, takes an `args` object whose entries must equal the call's arguments; `{ "tool": "get_mlb_standings", "args": {} }` allows every call of that tool.
...
Today the CLI has `serve`, `connect`, `conjure` (with `--model`), `model`, `tell`, `approve`, `deny`, and `version`. REST covers daemons (`POST /daemons` with `model`, `GET` and `PATCH /daemons/{name}`), messages, thread replies, approval requests and answers, allowlist matching, operation claims, markers, soul and memory, schedules, hosts, and the Telegram webhook. The SDK connects over Inngest Connect and builds one OpenAI Agents JS agent per turn, with each model call and tool call behind its own step. The daemon row's `model` picks the provider, with `AETHER_MODEL` as the fallback: OpenAI directly, Anthropic through the AI SDK adapter, or `scripted` offline. An unmatched tool call pauses the run; the daemon posts the calls plus the serialized run state as one approval group with a row per call, the run ends, and once every row in the group is answered a single `aether/approval.answered` starts a new run that restores that state, applies every decision, and continues (or pauses again with a new group). Every tool execution claims its operation ID first. The model answers with a status and text: `working` continues the run up to three times before a `turn.incomplete` marker, `done` posts the text verbatim as the only reply, and an empty reply leaves a `turn.empty` marker. Then a tool-less agent reads a plain-text transcript of the turn and rewrites memory against the version loaded at start; a failed extraction leaves a `memory.failed` marker without failing the turn. The `shell` tool runs with a scrubbed env inside the thread directory, stale schedules leave a marker, and `defineDaemon({ model, tools })` swaps either. Schedules carry a `prompt` that a fired turn delivers as its request; the daemon manages its own with `schedule_list`, `schedule_put`, and `schedule_delete`, and each turn's instructions state the date, time, and timezone. A turn also receives recent thread history from `GET /threads/{id}/messages`, budgeted by bytes, so conversations carry context without leaning on memory. File access is first class: `read_file` pages by line, `edit_file` makes one exact replacement, and `write_file` writes through a temp file, all path-matched by the allowlist and refused outside the thread directory. MCP servers attach with `mcp({ name, command, args })` (stdio, scrubbed env) or `mcp({ name, url })` (streamable HTTP); every listed tool runs through the same allowlist match, operation claim, and step as the built-ins, with output capped at 32000 chars, and `args` rules allow them without approval. Verified 2026-09-06 against the local `mlb-api-mcp` server: 24 tools listed over stdio and `get_mlb_standings` returned through the wrapper.
