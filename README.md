...
`--allowlist file.json` lists tool calls that run without approval. Anything unmatched becomes an approval row. A `shell` rule gives an `argv` prefix; a file-tool rule gives a `path` glob relative to the thread directory, so `{ "tool": "read_file", "path": "**" }` lets a daemon read anything there. Globs match dotfiles too; scope the rule to a subtree (`notes/**`) to keep a root `.env` behind approval.
