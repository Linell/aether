...
| `aether/approval.answered` | aether -> daemon | daemon, thread, group, decisions | Resume a paused turn once every call in its approval group is answered. |
...
- Approval pause is REST, not an event. The daemon posts the pending calls, the paused run's state, and its run id to `POST /threads/{id}/approvals`; aether stores one approval group per run (`INSERT OR IGNORE` on the run id) holding the state, plus one approval row per call pointing at the group, and emits `daemon/approval.requested` only when the group is new. A later pause for a still-pending call moves it to the new group.
...
- The approval group carries the paused run's state as an opaque blob. Aether stores and returns it (`GET /approvals/groups/{id}`) and never reads inside. Each call keeps its own approval row so any surface answers one call at a time; when the last pending row in a group is answered, the group is marked decided and one `aether/approval.answered` with every decision goes out in the same transaction. The resumed run applies every decision that matches an interruption; a group that is not decided or has no matching call leaves an `approval.ignored` marker.
